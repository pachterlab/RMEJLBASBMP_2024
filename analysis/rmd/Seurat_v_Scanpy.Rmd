...
```{r}
# norm_mat <- seu_celltypist[["RNA"]]@layers$data   # log-normalized counts (dgCMatrix)
norm_mat <- GetAssayData(seu_celltypist, assay = "RNA", layer = "data")  # slot = "data"
...
write.table(seu_celltypist[["RNA"]]@meta.data$gene_symbol, "genes.tsv", quote = FALSE, col.names = FALSE, row.names = FALSE)
```
...
# adata_seu_celltypist = ad.read_h5ad("adata_seu_celltypist.h5ad")
...
```{r}
celltype_ari_value <- mclust::adjustedRandIndex(seu_celltypes, scan_celltypes)
print(celltype_ari_value)

wompwomp::plot_alluvial(df = py$celltype_df, graphing_columns = c("seu_celltype", "scan_celltype"))
```
