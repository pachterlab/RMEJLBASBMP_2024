...
Select yaml file
```{r}
yaml_file <- "Fig4_Supp_Fig3_Supp_Fig15_Supp_Fig16"  # Fig1_Fig4, Supp_Fig2, Fig4_Supp_Fig3_Supp_Fig15_Supp_Fig16, Supp_Fig4, Supp_Fig5, Supp_Fig20, Supp_Fig20_DE, Supp_Fig21, Supp_Fig21_DE
do_celltyping <- TRUE
```
...
if (analysis_methods == "default" || analysis_methods == "seurat_like") {
    ...
    seu_hvg_nfeatures <- 2000
    seu_vars_to_regress <- NULL
    seurat_scale_max <- 10
    seu_n_neighbors <- 20
    # use newly-generated knn graph for umap
...
if (yaml_file == "Fig1") {
    scanpy_normalization_target_sum <- NULL
} else {
    scanpy_normalization_target_sum <- 1e4
}
```
...
if (seu_matrix_generation_method == "kb") {
    ...
    gene_symbols_seu <- readLines(file.path(seu_data_path, "cells_x_genes.genes.names.txt"))
} else if (seu_matrix_generation_method == "cellranger") {
    res_mat <- Read10X(seu_data_path, gene.column = 1)
    gene_symbols_seu <- rownames(Read10X(seu_data_path, gene.column = 0))
} else {
...
gene_symbols <- gene_symbols_seu
genes_kept <- rownames(seu)
df_meta <- data.frame(gene_id = gene_ids, gene_symbol = gene_symbols, stringsAsFactors = FALSE)
df_meta <- df_meta[match(genes_kept, df_meta$gene_id), ]
seu[["RNA"]]@meta.data$gene_id     <- df_meta$gene_id
seu[["RNA"]]@meta.data$gene_symbol <- df_meta$gene_symbol

# seu[["RNA"]]@meta.features$gene_symbol <- gene_symbols_seu
```
...
if scan_matrix_generation_method=="kb":
    ...
    symbols = pd.read_csv(f"{scan_data_path}/cells_x_genes.genes.names.txt", header=None, sep='\t').iloc[:, 0].values
...
JUST FOR SUPPFIG20, where Scanpy strangely has 1 more cell than Seurat - make sure the cell lists are the same (intersection)
```{r}
if (yaml_file == "Supp_Fig20" | yaml_file == "Supp_Fig20_DE") {
    seu <- subset(seu, cells = overlapping_inds)
    py$adata <- py$adata[py$adata$obs_names$isin(overlapping_inds), ]
    
    # repeat the code above, minus the overlapping_inds
    seu_inds <- colnames(seu)
    seu_genes <- rownames(seu)
...
seu_celltypist <- unserialize(serialize(seu, NULL))
```

```{python}
adata_scan_celltypist = adata.copy()
if r.scanpy_hvg_flavor != "seurat_v3":
    sc.pp.normalize_total(adata, target_sum=r.scanpy_normalization_target_sum)
```
...
Find HVGs
```{r}
seu <- FindVariableFeatures(seu, verbose = FALSE, selection.method = seurat_hvg_flavor, mean.cutoff = seu_mean_cutoff, dispersion.cutoff = seu_dispersion_cutoff, nfeatures = seu_hvg_nfeatures)
...
if (!identical(seu_inds, scan_inds)) {
    rownames(sc_nn_b) <- as.vector(py$adata$obs_names$values)  # NEW AS OF 2025 - can be moved outside this conditional (to match scan vs scan notebook), but since this is a new edition, I am keeping it in here to be safe
    colnames(sc_nn_b) <- as.vector(py$adata$obs_names$values)  # NEW AS OF 2025 - can be moved outside this conditional (to match scan vs scan notebook), but since this is a new edition, I am keeping it in here to be safe
...
UMAP
```{r}
seu <- RunUMAP(seu, dims = 1:seu_num_pcs, min.dist = seu_umap_min_dist, umap.method = seu_umap_method, seed.use = umap_seed_seu, metric = seu_umap_metric)
# DimPlot(seu, reduction = "umap") + scale_color_manual(values = ditto_colors)
```
...
umap_plots <- plot_umap(group1_umap_info = seu$umap@cell.embeddings, group1_clusters = seu$seurat_clusters, group2_umap_info = py$adata$obsm["X_umap"], group2_clusters = scanpy_clusters_renumbered, colors_group2 = colors_group2, save = c(file_paths$umap_seu, file_paths$umap_scan))
...
```{r}
seu_umap_data <- seu$umap@cell.embeddings
...
if r.do_celltyping:
    ...
    # ensure gene symbols are index
    adata_scan_celltypist.var["gene_ids"] = adata_scan_celltypist.var.index
    adata_scan_celltypist.var = adata_scan_celltypist.var.set_index("gene_symbols")
    adata_scan_celltypist.var_names = adata_scan_celltypist.var.index
    
    # log1p-normalize with target sum=1e4
    sc.pp.normalize_total(adata_scan_celltypist, target_sum=1e4)
    sc.pp.log1p(adata_scan_celltypist)
    
    # # use my own neighborhood graph
    if not all(adata_scan_celltypist.obs_names == adata.obs_names):
        adata_scan_celltypist = adata_scan_celltypist[adata.obs_names, :].copy()
    
    adata_scan_celltypist.obsp["connectivities"] = adata.obsp["connectivities"]
    adata_scan_celltypist.obsp["distances"] = adata.obsp["distances"]
    adata_scan_celltypist.uns["neighbors"] = adata.uns["neighbors"]
    
    # sc.pp.highly_variable_genes(adata_scan_celltypist)
    # adata_scan_celltypist.layers["scaled"] = adata_scan_celltypist.X.toarray()
    # sc.pp.regress_out(adata_scan_celltypist, ["total_counts", "pct_counts_mt"], layer="scaled")
    # sc.pp.scale(adata_scan_celltypist, max_value=10, layer="scaled")
    # sc.pp.pca(adata_scan_celltypist, layer="scaled", svd_solver="arpack")
    # sc.pp.neighbors(adata_scan_celltypist, n_neighbors=10, n_pcs=40)
    
    # run model
    predictions = celltypist.annotate(adata_scan_celltypist, model=r.model_pkl, majority_voting=True)
    pred_labels = predictions.predicted_labels[['majority_voting']]
    pred_labels = pred_labels.reindex(adata_scan_celltypist.obs_names)  # reorder to match adata_real.obs index
    adata_scan_celltypist.obs['celltype'] = pred_labels['majority_voting'].values
    
    # adata_scan_celltypist.write("adata_scan_celltypist.h5ad)
```

convert R into anndata for running celltypist
```{r}
if (do_celltyping) {
    # norm_mat <- seu[["RNA"]]@layers$data   # log-normalized counts (dgCMatrix)
    norm_mat <- GetAssayData(seu, assay = "RNA", layer = "data")  # slot = "data"
    
    # write matrix
    Matrix::writeMM(norm_mat, file = "norm_counts.mtx")
    
    # write barcodes and genes
    write.table(colnames(norm_mat), "barcodes.tsv", quote = FALSE, col.names = FALSE, row.names = FALSE)
    
    # write.table(rownames(norm_mat), "genes.tsv", quote = FALSE, col.names = FALSE, row.names = FALSE)
    write.table(seu[["RNA"]]@meta.data$gene_symbol, "genes.tsv", quote = FALSE, col.names = FALSE, row.names = FALSE)
    
    # Extract graphs
    snn <- seu@graphs$RNA_snn  # Shared nearest neighbor graph
    knn <- seu@graphs$RNA_nn   # KNN distance/adjacency graph
    
    # # Force dgCMatrix (required by writeMM)
    # snn <- as(snn, "dgCMatrix")
    # knn <- as(knn, "dgCMatrix")
    
    # Ensure symmetry for SNN (Scanpy expects symmetric connectivity matrix)
    snn_sym <- pmax(snn, t(snn))
    
    # -------- Normalize SNN rows (Scanpy-style row-normalized connectivities) --------
    row_sums <- Matrix::rowSums(snn_sym)
    row_sums[row_sums == 0] <- 1
    snn_norm <- snn_sym / row_sums
    
    # Convert to CSR using scipy.sparse
    Matrix::writeMM(snn_norm, "seu_connectivities.mtx")
    Matrix::writeMM(knn,      "seu_distances.mtx")
}
```

Celltypist on Seurat
```{python}
if r.do_celltyping:
    adata_seu_celltypist = sc.read_mtx("norm_counts.mtx").T
    adata_seu_celltypist.var_names = [x.strip() for x in open("genes.tsv")]
    adata_seu_celltypist.obs_names = [x.strip() for x in open("barcodes.tsv")]
    
    # ---- Load connectivities + distances exported from R ----
    snn = mmread("seu_connectivities.mtx")
    knn = mmread("seu_distances.mtx")
    
    # Convert to CSR (Scanpy format)
    adata_seu_celltypist.obsp["connectivities"] = csr_matrix(snn)
    adata_seu_celltypist.obsp["distances"]      = csr_matrix(knn)
    
    # Register Scanpy neighbor metadata (required for UMAP/leiden to work)
    adata_seu_celltypist.uns["neighbors"] = {
        "connectivities_key": "connectivities",
        "distances_key": "distances",
        "params": {
            "method": "seurat",
            "n_neighbors": r.seu_n_neighbors,
            "n_pcs": r.seu_num_pcs
        }
    }
    
    predictions_seu = celltypist.annotate(adata_seu_celltypist, model=r.model_pkl, majority_voting=True)
    pred_labels_seu = predictions_seu.predicted_labels[['majority_voting']]
    pred_labels_seu = pred_labels_seu.reindex(adata_seu_celltypist.obs_names)  # reorder to match adata_real.obs index
    adata_seu_celltypist.obs['celltype'] = pred_labels_seu['majority_voting'].values
    
    # adata_seu_celltypist.write("adata_seu_celltypist.h5ad)
    os.remove("norm_counts.mtx")
    os.remove("genes.tsv")
    os.remove("barcodes.tsv")
    os.remove("seu_connectivities.mtx")
    os.remove("seu_distances.mtx")
```

Plot it
```{python}
if r.do_celltyping:
    # adata_scan_celltypist = sc.read_h5ad("adata_scan_celltypist.h5ad")
    # adata_seu_celltypist = sc.read_h5ad("adata_seu_celltypist.h5ad")
    
    # intersection of shared barcodes
    shared = adata_scan_celltypist.obs_names.intersection(adata_seu_celltypist.obs_names)
    
    # extract aligned vectors
    scan_celltypes = adata_scan_celltypist.obs.loc[shared, "celltype"]
    seu_celltypes  = adata_seu_celltypist.obs.loc[shared, "celltype"]
    
    celltype_df = pd.concat(
        [
            seu_celltypes.rename("Seurat"),
            scan_celltypes.rename("Scanpy")
        ],
        axis=1
    )
    
    # celltype_df.to_csv("celltypes.csv")
```

```{r}
if (do_celltyping) {
    # df <- read.csv("celltypes.csv", header = TRUE, stringsAsFactors = FALSE)
    df <- py$celltype_df
    
    print(mclust::adjustedRandIndex(df$Seurat, df$Scanpy))  # ARI
    print(mean(as.character(df$Seurat) == as.character(df$Scanpy)))  # fraction of cells that agree
    
    wompwomp::plot_alluvial(df = df, graphing_columns = c("Seurat", "Scanpy"), sorting_algorithm = "none", color_bands = TRUE, coloring_algorithm = "left", text_size = 15, axis_text_size = 25, axis_text_vjust = 0, min_text = 1e-12, output_plot_path = glue::glue("{output_base_path}/plots/celltype_alluvial.pdf"))
}
```
Done with CellTypist
...
seurat_vs_scanpy_pvaladj_scatterplot <- plot_scatterplot_de_wilcoxon(markers2, save = file_paths$wilcoxon_scatterplot_file_path, outliers_excluded = FALSE)  # if I want to add spearman here, simply add the following argument: spearman = markers2$spearman[1]
...
```{r}
if (yaml_file != "Fig4_Supp_Fig3_Supp_Fig15_Supp_Fig16") {
    sessionInfo()
    stop("Additional biological analysis is only conducted on the conditions in Supp_Fig3 in the manuscript. It requires aligned clusters, so only the conditions from Supp_Fig3, Supp_Fig4, or Supp_Fig5 are suitable.")
}
```
...
try:
    ...
    import subprocess
    print("gget is not installed. Installing it now...")
    subprocess.check_call(["pip", "install", "gget==0.29.0", "--no-deps"])
    print("gget has been installed successfully!")
```

```{r}
gget <- import("gget")
output_base_path_biological_output <- glue::glue("{output_base_path}/additional_biological_output")
dir.create(output_base_path_biological_output, recursive = TRUE, showWarnings = FALSE)
ensembl <- FALSE  # for the PBMC 10k data, this requires converting to gene symbol before the fact, which is important because gget enrichr might throw an error for large ensembl ID lists
# markers2 <- readRDS(output_data_file_paths$markers2)  # for loading in the object after the fact
# markers2 <- markers2 %>% sample_n(200)  # for debugging with small data
```

```{r}
# Call the function with markers2, output base path, and custom dpi
source(glue("{project_base_path}/scripts/plotting_and_stats.R"))
for (package in c("Seurat", "Scanpy")) {
    create_volcano_plots(
      data = markers2,
      package = package,
      output_base_path = output_base_path_biological_output,
      dpi = dpi
    )
}
...
```{r}
# source(glue("{project_base_path}/scripts/plotting_and_stats.R"))
make_volcano_jaccards_and_upsets(markers2, make_upset_plots=TRUE)
```

Map from Ensembl --> gene symbols (gget enrichr only works with gene symbols) - write this file if it doesn't already exist
```{r}
# unique_genes_text_file_path <- glue("{output_base_path_biological_output}/unique_genes.txt")
# id_to_symbol_csv_path <- glue("{output_base_path_biological_output}/id_to_symbol.csv")
#     
# if (!ensembl && !file.exists(id_to_symbol_csv_path)) {
#     if (!file.exists(unique_genes_text_file_path)) {
#         unique_genes <- unlist(list(unique(markers2$gene)))  # make a list of unique Ensembl gene IDs from markers2
#         writeLines(unique_genes, unique_genes_text_file_path)
#     }
#     system(glue("python3 ../scripts/ensembl_to_symbol.py -i {unique_genes_text_file_path} -o {id_to_symbol_csv_path}"))
# }
```

make a gene symbol column in markers2
```{r}
if (!ensembl) {
    # # Load the CSV
    # gene_dict_df <- read.csv(id_to_symbol_csv_path, stringsAsFactors = FALSE)
    # 
    # # Convert to a named vector (dictionary)
    # gene_dict <- setNames(gene_dict_df$Gene_Symbol, gene_dict_df$Ensembl_ID)
    # 
    # markers2 <- markers2 %>%
    #   dplyr::mutate(gene_symbol = purrr::map_chr(gene, ~ gene_dict[[.x]] %||% NA_character_))
    

    # var_df <- py$adata$var %>% as.data.frame()
    # names(var_df)[names(var_df) == "gene_symbols"] <- "gene_symbol"
    # var_df$gene_ID <- rownames(var_df)
    var_df <- seu[["RNA"]]@meta.data[, c("gene_id", "gene_symbol")]
    markers2 <- markers2 %>% left_join(var_df[, c("gene_id", "gene_symbol")], by = c("gene" = "gene_id"))
}
```

run gget enrichr
```{r}
databases <- c("pathway")  # c("pathway", "ontology", "celltypes")
# gene_name_column <- if (ensembl) markers2$gene else markers2$gene_symbol
# background_list <- as.list(unique(gene_name_column))
source(glue("{project_base_path}/scripts/plotting_and_stats.R"))
make_volcano_go_plots(markers2, databases=databases, custom_background_list=NULL, ensembl=ensembl, save_kegg=FALSE, label_scatterplot_points=FALSE)  # custom_background_list=background_list
```
