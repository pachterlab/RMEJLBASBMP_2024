...
Select yaml file
```{r}
yaml_file <- "Fig1_Fig4"  # Fig1_Fig4.yaml, Supp_Fig2, Fig4_Supp_Fig3_Supp_Fig15_Supp_Fig16, Supp_Fig4, Supp_Fig5, Supp_Fig20, Supp_Fig20_DE, Supp_Fig21, Supp_Fig21_DE
do_celltyping <- TRUE
```
...
if r.do_celltyping:
    ...
    if not all(adata_scan_celltypist.obs_names == adata_seu_celltypist.obs_names):
        adata_scan_celltypist = adata_scan_celltypist[adata_seu_celltypist.obs_names, :].copy()
    ...
    # run model
    predictions = celltypist.annotate(adata_scan_celltypist, model=model_pkl, majority_voting=True)
...
if r.do_celltyping:
    ...
    predictions_seu = celltypist.annotate(adata_seu_celltypist, model=model_pkl, majority_voting=True)
...
if r.do_celltyping:
    ...
    celltype_df = pd.concat(
        [
            seu_celltypes.rename("seu_celltype"),
            scan_celltypes.rename("scan_celltype")
        ],
        axis=1
    )
...
if (do_celltyping) {
    ...
    print(mclust::adjustedRandIndex(df$seu_celltype, df$scan_celltype))  # ARI
    print(mean(df$seu_celltype == df$scan_celltype))  # fraction of cells that agree
    
    wompwomp::plot_alluvial(df = df, graphing_columns = c("seu_celltype", "scan_celltype"), sorting_algorithm = "none", coloring_algorithm = "left", text_size = 15, axis_text_size = 25, axis_text_vjust = 0, min_text = 1e-12, output_plot_path = glue::glue("{output_base_path}/plots/celltype_alluvial.pdf"))
}
```
...
try:
    ...
    subprocess.check_call(["pip", "install", "gget==0.29.0"])
...
make_volcano_go_plots(markers2, databases=databases, custom_background_list=NULL, ensembl=ensembl, save_kegg=FALSE)  # custom_background_list=background_list
```
